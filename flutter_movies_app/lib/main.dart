...
Future<Config> _loadConfig(Logger log) async {
  ...
  try {
    ...
    return Config(apiHost: config["apiHost"] as String, apiKey: config["apiKey"] as String);
  } catch (e) {
    log.e("Error while loading project configuration. Please make sure tha the file located at /assets/config/config.json exists and contains the correct configuration", error: e);
    rethrow;
  }
}